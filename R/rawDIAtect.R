...

{
  ...
  for(i in filenames){
    ...
    crayon::bold(paste("Loading",i,sep = " "))
    cat("\n")
    ...
    quant_ARO_small_sort_uni_sep <- tidyr::separate_rows(quant_ARO_small_sort_uni,Genes,sep=";",convert = TRUE)
    ...
    ggplot_drug_class <- as.tbl(unique(tidyr::separate_rows(ggplot_drug_class,drug_class,sep=";",convert = TRUE)))
