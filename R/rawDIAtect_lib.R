...
rawDIAtect_lib <- function(path_in = "",
                           pep_filter = 3,
                           path_out = "",
                           Exp_name = "")
{
  ...
  path_db_in <-
    system.file("data", "aro_index_DIA.tsv", package = "rawDIAtect")
  ...
  for (i in filenames_tsv)
  {
    ...
    file_print <-
      paste(file_count, "splitted Quant files saved to", sep = " ")
    ...
    df_matches_filt <-
      as.data.frame(filter(df_matches, matches == "FALSE"))
    list_needed_csv <- as.character(df_matches_filt$names)

    # Save
    lapply(list_needed_csv, function(x) {
      pb$tick()
      write_csv(spt2[[x]], path = paste(
        out_dir,
        x,
        paste("_", Exp_name, "_quant.csv", sep = ""),
        sep = ""
      ))
    })
  ...
  pb <-
    tcltk::tkProgressBar(
      title = "Total Progress of rawDetect",
      label = "0% done",
      min = 0,
      max = 100,
      initial = 0
  ...
  cat((
    crayon::bold$bgCyan("Starting analyzing with following options:", "\n")
  ))
  ...
  for (i in filenames) {
    ...
    filepath <- "C:/Users/blumenscheitc/Desktop/test/split/1615_2019_ZBS6_AMR_281-15__quant.csv"
    ...
    cat(crayon::bgGreen$black$bold(head(
      paste("Starts analyzing", head(paste(temp_dat[["Sample"]]), 1), sep = " "), 1
    )))
    cat("\n")
    cat("\n")

    # greps only ARO tags from temp
    quant_ARO <- temp_dat[grep("AMR-related", temp_dat[["Genes"]]),]

    # only dplyr::selects (Genes, Stripped.Sequence, Precursor.Quantity)
    quant_ARO_small <-
      dplyr::select(quant_ARO,
                    c(
                      "Sample",
                      "Genes",
                      "Stripped.Sequence",
    ...
    quant_ARO_small_sort <-
      quant_ARO_small[order(quant_ARO_small$Stripped.Sequence,
                            -abs(quant_ARO_small$Precursor.Quantity)), ]

    # removes duplicate PQ.seq keeps highest PQ.quant
    quant_ARO_small_sort_uni <-
      quant_ARO_small_sort[!duplicated(quant_ARO_small_sort$Stripped.Sequence),]

    # seperate multiple aro hits
    quant_ARO_small_sort_uni_sep <-
      tidyr::separate_rows(quant_ARO_small_sort_uni,
                           Genes,
                           sep = ";",
                           convert = TRUE)
    quant_ARO_sep <-
      quant_ARO_small_sort_uni_sep[grep("AMR-related", quant_ARO_small_sort_uni_sep$Genes),]

    # merge with index for correct gene names
    quant_ARO_sep_names <-
      merge(
        x = quant_ARO_sep,
        y = aro_index_DIA[, c("Genes",
                              "Names",
                              "amr_gene_family",
                              "Subfamily",
                              "Note",
    ...
    names(quant_ARO_sep_names)[names(quant_ARO_sep_names) == "Genes.y"] <-
      "Genes"
    quant_ARO_sep_names$Genes.x <- NULL

    # count amr_gene_family
    quant_ARO_sep_uni_pep <-
      quant_ARO_sep_names[!duplicated(quant_ARO_sep_names$Stripped.Sequence),]
    quant_ARO_count <-
      quant_ARO_sep_uni_pep %>% group_by(amr_gene_family) %>% add_count(amr_gene_family, name = "peptides")


    if (nrow(quant_ARO_count) == 0) {
      mock_data <- data.frame(matrix(ncol = 10, nrow = 0))
      col_name_mock <-
        c(
          "amr_gene_family",
          "Stripped.Sequence",
          ...
          "Note",
          "peptides",
          "Precursor.Quantity.Top3"
        )
      colnames(mock_data) <- col_name_mock
      mock_data[nrow(mock_data) + 1, ] = c("Na", "Na", head(paste(temp_dat[["Sample"]]), 1), 0, "Na", "Na", "Na", "Na", 0, 0)
      ...
      quant_top3_mean <-
        aggregate(quant_top3[, 3], list(quant_top3$amr_gene_family), mean)  #mean of group
      names(quant_top3_mean)[names(quant_top3_mean) == "Group.1"] <-
        "amr_gene_family"
      quant_ARO_index_top3 <-
        merge(
          x = quant_ARO_count,
          y = quant_top3_mean[, c("amr_gene_family", "Precursor.Quantity")],
          by = "amr_gene_family",
          all.x = TRUE
        )

      # sort data
      names(quant_ARO_index_top3)[names(quant_ARO_index_top3) == "Precursor.Quantity.y"] <-
        "Precursor.Quantity.Top3"
      names(quant_ARO_index_top3)[names(quant_ARO_index_top3) == "Precursor.Quantity.x"] <-
        "Precursor.Quantity"


      if (pep_filter > 1)
      {
        quant_ARO_index_top3_filt <-
          quant_ARO_index_top3 %>% filter(quant_ARO_index_top3[["peptides"]] > pep_filter)
      } else {
        quant_ARO_index_top3_filt <- quant_ARO_index_top3
      }

      # gets iso_names

      quant_ARO_iso_count <-
        quant_ARO_index_top3_filt %>% group_by(amr_gene_family) %>% add_count(Names, name = "count_name")
      ...
      quant_ARO_iso_count_diff <-
        quant_ARO_iso_count_max %>% mutate(iso_count_diff = max_count_name - count_name)
      iso_name_best <-
        quant_ARO_iso_count_diff %>% filter(as.numeric(iso_count_diff) <= 2)
      iso_name_best_small_uni <-  unique(iso_name_best)

      ggplot_filter_small_drugs <-
        as.data.frame(iso_name_best_small_uni)

    }

    # if no pattern found print mock found for visulication
    if (nrow(ggplot_filter_small_drugs) == 0) {
      ggplot_filter_small_drugs[nrow(ggplot_filter_small_drugs) + 1, ] <-
        NA
    ...
    {
      group.fills <-
        gameofthrones::got(length(unique(ggplot_input$amr_gene_family)), option = "Daenerys")
    ...
    ggplot_pep_count <-
      unique(dplyr::select(ggplot_input, c(
        "Sample", "amr_gene_family", "peptides"
      )))
    ggplot_pep_names <-
      unique(dplyr::select(
        ggplot_input,
        c("Sample", "amr_gene_family", "Names", "Subfamily", "Note")
      ))
    ggplot_pep_count_names <-
      ggplot_pep_names %>% group_by(Names) %>% add_count(Names, name = "count_1")
    count_amr_name <-
      ggplot_pep_count_names %>% group_by(amr_gene_family) %>% add_count(amr_gene_family, name = "freq")
    ggplot_names <-
      count_amr_name %>% mutate(perc = round((count_1 / freq) * 100, 1))

    p1 =
      ggplot() +
      geom_bar(
        data = ggplot_pep_count,
        aes(
    ...
    ggplot_PQ_amr <-
      unique(dplyr::select(
        ggplot_input,
        c("Sample", "amr_gene_family", "Precursor.Quantity.Top3")
      ))
    ...
    suppressMessages(main_page_pep <-
                       left_join(main_page_dat, ggplot_pep_count))
    suppressMessages(main_page_pep_PQ <-
                       left_join(main_page_pep, ggplot_PQ_amr))
    suppressMessages(main_page_pep_PQ_names <-
                       left_join(main_page_pep_PQ, ggplot_names))

    main_page_pep_PQ_names <-
      main_page_pep_PQ_names %>% mutate(Found_AMR = ifelse(Sample == "", "", amr_gene_family))
    ...
    ggplot_drug_class <-
      dplyr::select(ggplot_input, c("Sample", "drug_class", "peptides"))
    ggplot_drug_class <-
      as.tbl(unique(
        separate_rows(
          ggplot_drug_class,
    ...
    if (is.na(ggplot_drug_class$drug_class)) {
      ...
      ggplot_drug_sum <-
        aggregate(
          as.numeric(as.character(ggplot_drug_class$peptides)),
          by = list(drug_class = ggplot_drug_class$drug_class),
          FUN = sum
        )
      ggplot_drug_perc <-
        ggplot_drug_sum %>% mutate(perc = round((x / sum(x)) * 100, 2))
      ggplot_drug_perc[["Sample"]] <-
        rep(head(ggplot_drug_class[["Sample"]], 1), nrow(ggplot_drug_perc)) # make new column

      pie_1 =
        ggplot(data = ggplot_drug_perc[order(ggplot_drug_perc$perc, decreasing =
                                               T), ],
               aes(x = "plot",
                   y = perc)) +
        geom_bar(aes(fill = reorder(drug_class, perc)),
                 stat = "identity",
                 position = "stack") +
    ...
    {
      out_dir <-
        paste(path_in, "Report", sep = "/")
    } else {
      out_dir <- paste(path_out, "Report", sep = "/")
    }
    ...
    final_path_tab_tsv <-
      paste(
        paste(final_path_tab, Exp_name, "Full_Report" , sep = "_"),
        "csv",
        sep = ".",
        collapse = NULL
      )

    if (exists('quant_ARO_iso')) {
      write.table(
        quant_ARO_iso,
        file = final_path_tab_tsv,
        row.names = FALSE,
        na = " ",
        sep = ";"
      )
    ...
    pdf_path <-
      paste(final_path, paste(Exp_name, "Report.pdf", sep = "_"), sep = "_")
    ...
    info <-
      sprintf("%d%% done", round((loop_count / length(filenames)) * 100))
    tcltk::setTkProgressBar(pb, (loop_count / length(filenames)) * 100, label =
                              info)
